...
keep if year >=1688 & year <=1816

    twoway (line Total_Prize_value year, lpattern(dash) lcolor(red)  cmissing(n)) ///
   (line FR_Prize_value year,  cmissing(n) lcolor(red)) ///
   (line Frenchincome year, cmissing(n) lcolor(blue) msize(tiny)), /*
  */ ytitle("tons of silver", axis(1)) scheme(s1color) /*
  */ legend(rows(3) order (1 "English income from all prizes" 2 "English income from French prizes" /*
  */ 3 "French income from privateering")) /*
  */ xlabel(1700(20)1820)
