...
*codebook value if partner_grouping=="Afrique" & export_import=="Imports" & hamburg_classification=="Coffee"
...
fillin exportsimport year partner_grouping direction hamburg_classification
...
duplicates drop year direction export_import partner_grouping hamburg_classification, force
*keep year direction export_import partner_grouping hamburg_classification value

replace source_type = "imputed" if _fillin==1 & value !=.

replace value =. if partner_grouping=="États-Unis d'Amérique" & year <=1778
...
bysort year export_import pays class: egen weight_total=max(forweight)
drop forweight
gen share = value/weight_total
*br if share >1 & share!=.
bysort export_import pays class direction: egen weight=mean(share)
...
bysort year export_import pays class: gen value_for_obs = value if direction=="total"
		
gen blink = value if direction !="total" 
replace blink=. if source_type=="National par direction (-)" &  ///
				(year==1749 | year==1751 | year==1777 )
bysort year export_import pays class: egen blouf=total(blink), missing
replace value_for_obs=blouf if value_for_obs==.
drop blink blouf
bysort year export_import pays class direction:replace value_for_obs=. if _n!=1
...
bys class export_import pays : egen max_admiss_pred = max(value)
...
foreach i of num 1/5{
	foreach j of num 1/`: word count `levels''{
		summarize lnvalue if class==`i' & pays==`j' & export_import=="`ciao'"
		if r(N)>1{
			qui reg lnvalue i.year i.dir [iw=weight] if ///
				export_import=="`ciao'" & pays==`j' & class==`i', robust 
				
			
			predict value_pred if ///
				export_import=="`ciao'" & pays==`j' & class==`i'
			...
			*have a look at imputed export data
			...
			twoway (connected pred_value year, msize(tiny) legend(label(1 "Predicted"))) ///
					...
					if pays==`j' & class==`i' & export_import=="`ciao'" & direction=="total", title(`ciao') ///
...
collapse (sum) pred_value, by(year export_import partner_grouping hamburg_classification)
