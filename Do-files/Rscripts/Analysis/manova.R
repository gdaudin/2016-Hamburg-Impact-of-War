...
composition_trade_test = function(df, period1, period2, plantation_yesno, direction, X_I, classification){
  ...
  df = df %>% group_by(year, war, product_sitc_simplEN=df[,which(colnames(df) == classification)], export_import, period_str) %>% 
...
collapse (sum) value, by(year war `classification' export_import period_str)
