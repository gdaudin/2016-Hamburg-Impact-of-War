...
indicate("Product FE= *.class" "Product time trend=*year_class*" "Product quadratic trend= *year2_class*") drop(0.* year `macro') \\\
pr2 nonumbers mtitles("All wars" "All wars" "War by war" "War by war") title(Regression table\label{tab1}) replace
...
esttab using "$thesis2/reg_table/hamburg2/lag1/lag1.tex",label booktabs longtable noomitted varlab( _cons "Constant") not \\\
 indicate("Product FE= *.class" "Product time trend=*year_class*" "Product quadratic trend=*year2_class*" ) \\\
 drop(0.* `macro1' `macro2' *each_class* *all_class* year) pr2 nonumbers mtitles("All wars" "All wars" "War by war" "War by war") \\\
