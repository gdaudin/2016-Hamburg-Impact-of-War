...
The numbers given by Starkey and Hillman for privateers’ prizes are not the same, but the correlation between the two series is 0.8.
...
Though it must be recognized that the sources are not completely comparable  \citep{Hillmann2011, p. 757, note 73}).
The list of ships that integrated the Royal Navy after captures has been compiled by \cite{Lyon1993} (provided by Henning Hillmann).
The list includes military ships, so we drop all the rated ships and the sloops.
We use data from \cite{Starkey1990} and \cite{Benjamin2009}. 
The former provides the number of prizes condemned as legal by the High Court of the Admiralty in London, both from privateers and the Royal Navy from 1702 to 1785
(The numbers given by Starkey and Hillman for privateers’ prizes are not the same, but the correlation between the two series is 0.87 ; it is only 0.66 between Starkey’s navy prize and what can be deducted from Lyon) while the latter provides the annual number of prizes taken by the Royal Navy from 1793 to 1815 from the sample gathered, but not published, by \cite{Hill1998}.
We assume
Data from these sources are summarised in figure \ref{Prizes}.
