...
In section \ref{sec:loss} we construct the loss function to observe the trend of French losses throughout the period in analysis. In this section we establish a link between the loss function and the most common strategies used to curtail trade. We aim to identify the channels used by the British to disrupt French trade and disentangle their mechanisms. We also provide a measure to quantify their success and ease the comparison with the loss function. We examine in detail the following four possible channels: 1) ship building and alliance making 2) capture of colonies 3) predation on shipping 4) predation on neutral.  

\subsection{Ship building and alliance making}
British naval supremacy over France meant that the British could blockade French ports and capture French ships. Naval superiority is a possible explanation to the success of British trade war, therefore we want to be able to quantify it and compare it with the loss function. \\
As a measure of the superiority of the British navy over its enemies we compare the number of warships available to Great Britain and its allies with that available to France and its allies.\footnote{data are provided in \cite{Modelski1988}}
...
As we saw in section \ref{sec:loss}, the most disruptive wars were the Seven Years War and the Revolutionary \& Napoleonic Wars. Overlying the loss function to the naval supremacy function therefore does not reveal a high level of correlation. 
\todo[inline, color = RoyalBlue]{Why dont we actually compute a correlation?}
As a consequence we argue that naval supremacy alone cannot explain the success of the British trade wars over the French.  As a matter of fact, in periods when the French navy was at its peak, the loss function is still high. 
...
\todo[inline, color =RoyalBlue]{I am sorry, my questions was how many "directions" are using in the regression? all of them or just the main ones? I wanted to make a clarifying table}
...
The peak in 1781 comes from the beginning of the 4th Anglo-Dutch War.  \todo[inline, color = RoyalBlue]{What do you think of this solution?}}
\end{minipage}
\end{figure}
