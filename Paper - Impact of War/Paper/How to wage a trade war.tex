...
We also provide a measure to quantify their success and ease the comparison with the loss function. \todo[inline, color = YellowGreen]{What do you mean ?} 
...
We examine in detail the following four possible channels: 1) ship building and alliance making 2) capture of colonies 3) predation on shipping 4) predation on neutral.  
 \todo[inline, color = YellowGreen]{What is the good order for presenting them ?}
...
This was easier to do if French warships could not interfere because of unfavorable odds.
...
\footnote{It has to be noted here that we cannot compute a proper correlation with the loss function because this measure only makes sense in war periods.}
 \todo[inline, color = YellowGreen]{But we could restrict the correlation to war periods?}
...
This could be because our proxy for naval superiorit is too coarse a measure.
For exemple, the French navy advantage during the Seven Years war is explained by the fact that Russia and Sweden were allied to France.
...
West Indian French colonies were major sources  of sugar, coffee and other colonial goods which imports and re-exports was the most dynamic part of French trade.
They were also important markets for French products.
The loss of these colonies was bound to be disruptive for French trade.
...
We argue that, despite the fact tloss of colonies was very heavy on French trade, and Saint Domingue in particular, this was not enough to make a successful trade war. In fact, the loss of colonies does not explain the peak in loss after the Seven Years War.  
 \todo[inline, color = YellowGreen]{This has to be bolstered by some quantitative analysis}.
...
Hence a direct way to affect French trade was to impair French see trade.
...
In fact, at its peak, the share of lost trade due to predation gets to 10\%.
What's more, this happens during the War of American Independence, while during the Revolutionary War it gets as low as 3\%.
 \todo[inline, color = YellowGreen]{Whose total trade ?}.
This is relevant when compared to our loss function, which reveals an inverse trend.
Neither an increase in number nor in value of prizes taken by the Royal Navy or priveteers during this period can explain the variations in the loss function. 
...
This left the door open to trade under neutral flag, especially for French merchant. 
...
It also created great discontent among neutral countries, which, during Seven Years War, experienced trade losses similar to those of France.
 \todo[inline, color = YellowGreen]{I do not understand the comparison. You do not mean total trade losses for neutral countries, as this cannot be mesured. So you cannot compare them with France?}.
 Figures \ref{fig:loss_by_war_status_XI} and \ref{fig:loss_by_war_status_X_I} show the loss function, as explained in section \ref{sec:loss}, for countries grouped by war status \footnote{more on the grouping by war status to be found in appendix \ref{app:historical_summary}} and provide evidence of how neutral countries, during the Seven Years Wars and the subsequent years, had losses sometimes higher than foes (this is the case for imports). 
  \todo[inline, color = YellowGreen]{Ok, that is clearer. But then remove the sentence before}
...
Their claim was that neutral ships traveling under the protection of neutral warships were not to be inspected as the absence of ``enemy cargo'' was guaranteed by the neutral sovereign \cite[p. 121-125]{Schnakenbourg2013}.
