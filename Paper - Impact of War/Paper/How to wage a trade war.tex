...
inflicted on the French economy, waging this war on trade was a costly endeavour.
\end{abstract}
...
Attacking and protecting trade are major military activities, such as during US Civil War, World War I and World War II (both in the Atlantic and the Pacific).
...
Long term changes in relative economic fortunes are more difficult to achieve for attacks on trade.
...
These feats have inspired generations of naval strategists such as Mahan.
How they were exactly achieved is not clear.
\todo[inline, color = RoyalBlue]{Develop. Some say the "guerre d’escadre" was decisive, some say it was the "guerre de course". La guerre d’escadre n’a pas en tant que telle des effets directs sur le commerce. La guerre de course décline durant la période, ce qui suggère qu’elle n’a pas été décisive non plus. Le Blocus continental aussi a pu jouer un rôle ?}


This paper examines the mechanisms explaining the success of the British war on French trade from 1744 to 1815.
...
Finally, the « Second Hundred Years War » (1688-1815) between France and Britain has a central place in the history of the eighteenth and the nineteenth century and deserves to be better understood in its own right.
...
First, a caveat.
War has many causes.
Still, especially after the consolidation of the Hanoverian dynasty on the United Kingdom’s throne, mercantile rivalry was an important motivation of French wars \citep{Wallerstein1980, Tracy1991, Davis2006, Crouzet2008}; each nation was jealous of the other's commercial success; the British state, at the urging of mercantile interests, believed war was a good way to curtail French trade, despite the huge resources it required \citep{Baugh1965, Neal1977, Brewer2002}.
...
Furthermore, before 1815, it is not obvious that French peacetime trade was much affected by British efforts: despite a clear decrease in wartime, French trade was recovering quite fast during the peace periods.
War by itself was not enough to curtail French trade.
By studying how the British war on French trade was waged, we can understand its mechanisms and what explained its eventual success.
...
The British economy has been extensively shaped by war before the establishment of the \emph{Pax Britannica} in the early nineteenth century.
...
\cite{Anderton2001} distinguish between conflicts with major and minor power and observe significant pre and post war effects only in the former case, whereas \cite{Levy2004} find a mild and temporary impact of war on trade with adversary country, using a panel between 1870 and 1992.
...
The claim that the indirect cost of forgone trade was at least as large as the direct effect of lost human capital.
...
\cite{Glaser2016} estimates that even the naval arm race by itself at the end of the nineteenth century (1870-1914) was damaged commercial interests and lowered trade potential around the world.
...
This paper discusses these channels and as such engages with a literature discussing mechanisms and strategies for the war on trade.
\cite{Tracy1991} aims at understanding the strategy of attacks on maritime trade based on historical evidence, mainly from World War I.
...
They judge that the success rate of blockades has not been very high.
\cite{Riley1986} underplays the role the Seven Years War on French trade because of the compensatory activity of merchants.
In addition to looking at war-time channels, this paper innovates by focusing on how Britain managed to change the long-term trends of French trade even after peace between Britain and France became firmly established.

The aim of this paper is [to write up]
Roadmap [to write up]

We aim to show it was a difficult and costly endeavor; the key factor was not to reduce the level of trade, but to force its structural change; it succeeded partly because the French cooperated through their own attempt at changing the structure of their trade, notably during the period of the Continental System.
...
we define a loss function that quantifies the disruption in French trade due to wars. We reckon this is an approximate measure (see section \ref{sec:cost_benefit}) however it provides a first idea of which wars were more disruptive and the length of their effects. 


%\subsection{Loss function}
\label{sec:loss}
Our data on French trade come from the \textit{Bureau de la Balance du Commerce} through the TOFLIT18 project (see appendix \ref{sec:dataset}). We construct the loss function as the difference between predicted trade value in the absence of war and the actual value of trade, divided by the predicted trade value:
\begin{equation}
\label{eqn:loss}
...
Other events than war could have affected French trade, notably the political upheavals linked to the French Revolution. 
Though wars were such massive events that so obviously affected French trade that the existence of a causal effect of wars on these trend deviations is quite plausible.
Anyway, disentangling the effects of war from the effects of the Revolution is probably impossible \citep{Charles2021}).
The same issue would apply to other periods where the evolution of French trade is multicausal.
For this reason, even if causality is not rigorously proven, we will use the language of causality in the rest of the paper, .
...
Furthermore, for the Revolutionary \& Napoleonic War and the Seven Years War, the loss stays positive also after the end of the conflicts, meaning the effects were long lasting, whereas it becomes negative after the Austrian Succession War and the American Revolutionary War. This implies that the disruption of French trade lasted through peacetime for the Revolutionary \& Napoleonic War and the Seven Years War. The right-hand panel in this figure reaffirms the point. We observe, in fact, that post-Seven Years War loss (during peace) is roughly comparable to the war-time loss during the American Revolutionary War altogether.
...
\todo[inline, color = NavyBlue]{Mahan believed in decisive battles and Blockade, not in the attack of enemy's trade. The \textit{Jeune école} believed attack on trade could be efficient (e.g. U-boats)}
In trade wars, many were the strategies implemented to disrupt the enemy's trade. For this work, we have gathered data on the most common techniques adopted and we provide here a quantification of their effect. 
We do so by creating a quantitative measure from the data and by linking it to the loss function, as constructed in section \ref{sec:loss}. 
We run six different univariate regressions for each strategy and one multivariate regression including all strategies. We first consider a linear model for war years only, then we do so considering discounted running sum\footnote{the discounted running sum is defined as $\sum_{t=1}^N value*e^{N-t}$} and finally we compute discounted sums for all years. We repeat this exercise in log, therefore estimating semi-elasticities. 
...
We examine in detail the following four possible strategies: 1) ship building and alliance making 2) capture of colonies 3) predation on shipping on neutral shipping 4) predation on french shipping. 
...
This was easier to do if French warships could not interfere because of unfavourable odds.
Naval superiority is thus the primary input into the British war on trade.
...
The regressions provide mostly insignificant coefficients and generally very small. In the linear case, the only significant coefficients are for the running sum cases (all years and war years only); one suggests an increase in the loss function of 0.043 percentage points for a one percentage point increase in the ration between navy sizes and the other a decrease even of 0.012 percentage points. As for the semi-elasticities, again there appears to be a decrease in the loss function for an increase in the ratio between navy sizes considering war years only and all years with the discounted sum, but an increase if we consider all years and no discounted sum. Overall the link with the loss function is always very mild, as suggested by the very low R squared, which never reaches 50\%. 
...
Still, the existence of the Russian and Swedish fleet had to be taken into account by the British navy. As a consequence, we feel confident to claim that, much as naval superiority was an important factor in a trade war, it is not enough to explain its success entirely and even then, the mechanisms at stake are not obvious. 
...
They were also important markets for French products, hence 
the loss of these colonies was bound to be disruptive. As a consequence,
the British steadily tried to erode the French colonial empire.
...
The revolt of the slaves in Haiti was a long and bloody episode and sugar production was lost to France well before independence was ultimately acquired.
...
According to it, the colonial losses during the Seven Years War were minimal (Canada was not an important trade partner for France).
In contrast, a large part of the French Empire was gradually lost in the Revolutionary \& Napoleonic Wars.
Computing a correlation between the colonial empire measure and the loss function we obtained a value of 30\% considering only war years and down to 4\% considering cumulative discounted losses during the whole period. There is no doubt the loss of Saint Domingue was decisive in the trade war. However, our goal is to identify the mechanisms that made it a successful strategy. 
...
\todo[inline, color = NavyBlue]{What do we want to say with this section?}
...
They had claimed from the seventeenth century the ``right of convoys'', that is ``immunity from search for neutral merchant vessels sailing under the convoy of a warship of the neutral'' \citep{TheEditorsofEncyclopaediaBritannica2014}.
 \todo[inline, color = RoyalBlue]{Citer Tracy plutôt ?}
...
It also created great discontent among neutral countries, whose loss function is nearly as high as that of a belligerent countries, as shown in figures \ref{fig:loss_by_war_status_XI} and \ref{fig:loss_by_war_status_X_I}.
 \todo[inline, color = NavyBlue]{Makes more sense?}.
...
Hence a direct way to affect French trade was to impair French sea trade.
...
Because direct destruction was rare and ransoming was limited, the overall value of prizes captured by the British navy and privateers provides a good measure of the pressure war-time predation exerted on trade.
...
For prizes captured by the British navy, we use data from \cite{Starkey1990} and \cite{Benjamin2009}. 
...
According to \cite{Hillmann2011}, this can be explained by the development of other war-time trade profit opportunities that made privateering a less interesting alternative for British merchants.
...
In 1813 the total number of prizes declines abruptly.
...
Another measure of the pressure British exercised on French trade comes from English and British trade statistics on the value of prize goods imported in Great Britain up to 1800 (see figure \ref{Prize goods imports}).
Unfortunately we could not gather data on the Blockade period.
Still, the numbers are very telling, especially when we look at the value of prizes as shares of total trade.

In fact, at its peak, the value of prizes captured gets to 10\% of the value of total French trade. 
What's more, this happens during the War of American Independence, while during the Revolutionary War it gets as low as 3\%.
 \todo[inline, color = NavyBlue]{Clearer ?}.
This is relevant when compared to our loss function, which reveals an inverse trend. Once again in fact a correlation between loss function and value of prizes imported by the British is either insignificant or close to zero. 
Neither an increase in number nor in value of prizes taken by the Royal Navy or priveteers during this period seem to explain the variations in the loss function. 
...
\todo[inline, color = YellowGreen]{Add 1792 data ? Start earlier to see the catch-up ?} 
