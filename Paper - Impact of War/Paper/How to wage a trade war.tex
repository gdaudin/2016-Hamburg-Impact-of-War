...
In trade wars, many were the strategies implemented to disrupt trade. Some were more efficient than other however, to the best of our knowledge, an explanation of what are the underlying mechanism has not yet been provided. With this paper we aim provide an answer to this question. 
In section \ref{sec:loss} we construct a loss function to quantify French trade losses throughout the period.
In this section we study the link between this loss function and the tools used by the British against French trade.
...
We examine in detail the following four possible strategies: 1) ship building and alliance making 2) capture of colonies 3) predation on shipping on neutral shipping 4) predation on french shipping. We create a measure for each one of the them and we quantify the degree to which they are related with the loss function.
...
As we saw in section \ref{sec:loss}, the most disruptive wars were the Seven Years War and the Revolutionary \& Napoleonic Wars. A correlation between the loss function and the ratio of warships reveals no significant link between these two phenomena, both considering only war years and using a discounted running sum which also includes peace years.\footnote{the discounted running sum is defined as $\sum_{t=1}^N value*e^{N-t}$}
Hence, naval superiority alone was not the ``secret ingredient'' that made a trade war successful. 
