...
\cite{Benjamin2009, Figure 1 and text)} provides the annual number of prizes taken by the Royal Navy from 1793 to 1815 from the sample gathered using the HCA 2/300 series, but not published, by \cite{Hill1998}. 
As noticed by \cite{Hillmann2011, p. 757, note 73}, Hill’s source appears to include all prize actions of Royal Navy warships, including prizes condemned in colonial courts or captured warships that are not included in the other records.
...
Data on the gross value of privateers’ prizes come from \cite{Hillmann2011}, reflated by McCusker’s Composite Commodity Price Index  \citep{McCusker2001, Table D-1, column 4} (reference year: 1700) to obtain nominal values.
...
For post-1793 Royal Navy prizes, we use the data from \cite{Benjamin2009, figure 2}, based on \cite{Hill1998}, reflated using McCusker’s Composite Commodity Price Index (reference year: 1804).
This is imperfect because Benjamin used the payment year rather than the capture year to deflate his numbers, but we do not have access to that data.
We have rescaled the graph to get a gross total of 30.8 million pounds (notice that Benjamin gives an net total of 26.6 million pounds). 
These data include warship prizes, which are more expensive than trade ships.
We have no basis to adjust this number.
Quite arbitrairly, we assume investment by British privateers was the same share of the value prizes as the French during their War of Austrian Succession (i.e. 59\%) and we adjust for the other wars using the prizes over vessels ration given by  \cite{Hillmann2011, table 6}. 
Data from these sources are summarised in figure \ref{Prizes_valàe}.
