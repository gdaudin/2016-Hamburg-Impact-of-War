...
\begin{abstract}
BLIF BLIF International trade is one of the main issues at stake in the rivalry between powers.
...
<<<<<<< HEAD
MORE RANDOM STUFFWe show that long term success implied a durable change in the structure of French trade.
Finally, we compute that, compared to loses inflicted on the French economy, waging this war on trade was a costly endeavor.
=======
BLOURKWe show that long term success implied a durable change in the structure of French trade.
Finally, we compute that, compared to loses inflicted on the French economy, waging this war on trade was a costly endeavor. RANDOM
>>>>>>> 1dae30dce17151f361038bd655857c16a03efe1e
\todo[inline]{to be re-written at some point -- I had a go! GD}
\end{abstract}
...
\todo[inline, color=blue!40]{Contrast the interest of the British merchants and the interest of the state as large}
\todo[inline, color=blue!40]{Discuss political cost of Neutral policy (Spain 1760, US 1812, League of armed Neutralty)}
\todo[inline, color=blue!40]{Discuss the fact that mercantile interests were not important enough to push to peace)}
...
\todo[inline, color = YellowGreen]{Is this second source just a double check with the first or it provides a different insight? -- GD do I need to go further? }
...
\todo[inline, color = YellowGreen]{Chronology of values or of number of prizes? could you clarify a bit what is added by Bejamanin to hill source? -- GD do I need to go further?}
...
\todo[inline, color = YellowGreen]{This graph should be redone omitting the peak in Danish prizes for readability and this paragraph above just goes in the note for the figure}
...
\todo[inline, color = YellowGreen]{The graph needs to be redone and eden traty part needs to be omitted}
...
We run individual regressions for each product because of collinearity of the explanatory variables (which are share, therefore highly correlated) and the little number of observations, which compromises the individual estimation of each coefficient.\todo[inline]{We need a better point here}.
...
\todo[inline, color=YellowGreen]{Could you write a quick two-lines on the takeway of the regression by geography? -- GD is it fine ?}
