...
We construct the loss function as the difference between predicted trade value in the absence of war and the actual value of trade, divided by the predicted trade value:
\begin{equation}
\label{eqn:loss}
...
In trade wars, many were the strategies implemented to disrupt the enemy's trade. For this paper, we have gathered data on the most common techniques adopted and we provide here a quantification of their effect. 
We do so by creating a measure from the data and by linking it to the loss function, as constructed in section \ref{sec:loss}. 
...
We examine in detail the following four possible strategies: 1) ship building and alliance making 2) capture of colonies 3) predation on French shipping. We also analyse the case of predation on neutral shipping, however we could not create a measure reliable enough to run a regression. 
...
As for the other war-time strategies we perform a regression on the loss function of the following covariates; prizes imports, number of prizes and number of privateer's prizes. 
...
It has to be noted here that the number of observations for prizes imports is very little (20 or 36, depending on whether we use the running sum) and therefore the estimates cannot be accurate. The coefficients for the linear regression in fact are not significant, except for the all-year and running sum case, where it is significant but negative (i.e. prizes import increase and the loss decrease, which is quite counter-intuitive). The R squared however never goes above 5 percent. The same happens with semi-elasticities, with the only significant coefficient being for all years and running sum and again negative. Again the R squared is quite low (at most 11 percent). As per what concerns number of prizes, coefficients in the linear specification are positive and significant except for the all-years running sum case. They vary substantially across specifications, ranging from 6 to 16 percent (so for a percentage point increase in number of prizes with respect to their maximum level there is an increase of 6 to 16 percentage points for the loss function). The R squared is also higher, between 20 and 40 percent. In the log-specification the picture change quite substantially. The coefficient range from 0.8 percent to 11 percent and R square is also between 5 and 40 percent. Finally, the regression on privateers' prizes is the most counter-intuitive. The coefficients are all very significant and all negative, which would imply privateer's action increase trade. This is true for all specifications with an R squared between 16 and 40 percent. We cannot really explain this result.
